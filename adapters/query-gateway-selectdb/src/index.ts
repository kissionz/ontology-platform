...
import type { QueryResult } from "../../../packages/contracts/src/index.js";
...
export interface SelectDbConfig { host:string;port:number;username:string;password:string;database:string;catalog?:string;tls?:boolean;connectionLimit?:number }

export class SelectDbGateway implements QueryExecutorPort {
  private pool?:Pool;
  constructor(private readonly config?:SelectDbConfig){}
...
export function selectDbConfigFromEnv(env:NodeJS.ProcessEnv=process.env):SelectDbConfig|undefined{if(!env.SELECTDB_HOST||!env.SELECTDB_USER||!env.SELECTDB_PASSWORD||!env.SELECTDB_DATABASE)return undefined;return {host:env.SELECTDB_HOST,port:Number(env.SELECTDB_PORT??9030),username:env.SELECTDB_USER,password:env.SELECTDB_PASSWORD,database:env.SELECTDB_DATABASE,catalog:env.SELECTDB_CATALOG,tls:env.SELECTDB_TLS==="true"};}
