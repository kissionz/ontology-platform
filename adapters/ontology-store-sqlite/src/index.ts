...
export class SqlitePlatformStore {
  ...
  matchValues(namespace: string, version: number, question: string, scope?: { objectIds?: string[]; propertyIds?: string[]; exact?: boolean }) {
    if (scope?.objectIds?.length === 0 || scope?.propertyIds?.length === 0) return [];
    const clauses = ["namespace=?", "ontology_version=?", "length(normalized_value)>0", scope?.exact ? "normalized_value=?" : "instr(?,normalized_value)>0"];
    const parameters: Array<string | number> = [namespace, version, normalize(question)];
    for (const [column, ids] of [["object_id", scope?.objectIds], ["property_id", scope?.propertyIds]] as const) if (ids) {
      clauses.push(`${column} IN (${ids.map(() => "?").join(",")})`); parameters.push(...ids);
    }
    return this.db.prepare(`SELECT object_id objectId,property_id propertyId,display_value displayValue,frequency FROM property_value_index WHERE ${clauses.join(" AND ")} ORDER BY length(normalized_value) DESC,frequency DESC,property_id,normalized_value LIMIT 1001`)
      .all(...parameters) as Array<{objectId:string;propertyId:string;displayValue:string;frequency:number}>;
  }
