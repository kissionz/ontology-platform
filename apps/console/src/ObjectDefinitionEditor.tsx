    ...
    {editing ? <>
      ...
      <div className="model-editor-body">
        {tab === "basic" && <div className="model-form-grid">
          ...
          <FormField label="对象编码"><input aria-label="对象编码" value={value.name} onChange={event => change({ name: event.target.value })} /></FormField>
        ...
        {tab === "properties" && <>
          ...
          {property && <section className="property-settings" aria-label="属性配置"><h3>{property.label}</h3><div className="model-form-grid">
            ...
            <FormField label="语义类型"><Choices label={`${property.label}语义`} value={property.meaning} values={MEANINGS} onChange={meaning => changeProperty({ meaning: meaning as OntologyProperty["meaning"], numericSpec: meaning === "NUMBER" ? property.numericSpec ?? { ...numericDefaults } : undefined })} /></FormField>
            ...
            {property.meaning === "NUMBER" && property.numericSpec && <fieldset className="numeric-settings wide"><legend>聚合语义</legend><div className="model-form-grid">
              <FormField label="数字类型"><Choices label={`${property.label}数字类型`} value={property.numericSpec.kind} values={["GENERAL", "CURRENCY", "RATIO"]} onChange={kind => changeProperty({ numericSpec: { ...property.numericSpec!, kind: kind as "GENERAL" | "CURRENCY" | "RATIO", ...(kind === "RATIO" ? { aggregationBehavior: "NON_ADDITIVE", defaultAggregation: "NONE" } : {}) } })} /></FormField>
            ...
            <details className="model-advanced wide"><summary>高级语义与访问设置</summary><div className="model-form-grid">
              <FormField label="语义匹配优先级"><input aria-label="属性匹配优先级" type="number" min={0} max={100} value={property.bindingPriority} onChange={event => changeProperty({ bindingPriority: Number(event.target.value) })} /></FormField>
