...
it("qualifies joined schema columns and preserves bound database names and table metadata", async () => {
  const query = vi.fn(async (sql: string, values: unknown[]) => {
    ...
    const references = [...sql.matchAll(/(?:(\w+)\.)?\b(TABLE_SCHEMA|TABLE_NAME|TABLE_TYPE|COLUMN_NAME|COLUMN_TYPE|IS_NULLABLE|COLUMN_COMMENT|ORDINAL_POSITION)\b/g)];
    ...
    return [[
      { tableName: "orders", tableType: "BASE TABLE", columnName: "id", dataType: "bigint", nullable: "NO", comment: "订单标识" },
  ...
  try {
    expect(await gateway.scanSchema()).toEqual([
      { name: "orders", type: "TABLE", columns: [{ name: "id", dataType: "bigint", nullable: false, comment: "订单标识" }, { name: "amount", dataType: "decimal(18,2)", nullable: true }] },
