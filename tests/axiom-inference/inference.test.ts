...
describe("A10-A12 deterministic inference",()=>{it("A10 derives store to business unit with two premises and proof",()=>{const result=runKernel(validSnapshot());const inferred=result.inferences.find(i=>i.predicate==="RELATION_REACHABLE"&&i.subjectId==="o_store"&&i.objectId==="o_bu");expect(inferred?.premiseAssertionIds).toEqual(["r_dept_bu","r_store_dept"]);expect(inferred?.proof.map(p=>p.kind)).toEqual(["FACT","FACT","AXIOM","DERIVATION"]);});it("A11 rejects hierarchy cycles",()=>{const s=validSnapshot();s.dimensionHierarchies[0]!.levels.push(s.dimensionHierarchies[0]!.levels[0]!);expect(runKernel(s).issues.some(i=>i.code==="HIERARCHY_TRANSITIVE"&&i.message.includes("环"))).toBe(true);});it("A12 is byte-for-byte deterministic",()=>{const a=runKernel(validSnapshot()),b=runKernel(validSnapshot());expect(a.inferences).toEqual(b.inferences);expect(a.inferenceDigest).toBe(b.inferenceDigest);});});
