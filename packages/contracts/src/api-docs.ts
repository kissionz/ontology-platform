...
export const API_DOCS = {
  ...
  ResolveOntologyContext: doc("解析语义上下文", "根据问题或术语一次获取匹配的对象、指标、值、公理和推理证据，供 Agent 理解业务语义。此接口不执行数据查询。", "semantic:read", "data 包含 sessionId、ontologyVersion、objects、metrics、relations、values、axioms、inferences、refs、ambiguities 和 contextDigest。会话固定版本，有效期 30 分钟。"),
...
export const REQUEST_DOCS: Record<string, { fields: Record<string, string>; example: unknown }> = {
  ResolveSemanticContextInput: { fields: { namespace: "本体命名空间，例如 retail。", ontologyVersion: "发布版本号或 latest；省略时选择最新发布版本。", question: "需要理解的自然语言问题；question 与 terms 至少提供一项。", terms: "明确的业务术语数组；可与 question 配合，二者至少提供一项。", purpose: "用途：ANSWER 回答、PLAN 规划、EXPLAIN 解释、MODEL 建模。", projection: "对象字段详细程度：compact（默认）、standard、full。敏感字段边界始终生效。", include: "开关：values 值匹配、axioms 公理、inferences 推论、evidence 证明过程；values 需显式开启，其他默认包含。" }, example: { namespace: "retail", ontologyVersion: "latest", question: "按店铺查看销售额", purpose: "PLAN", include: { values: true, axioms: true, inferences: true, evidence: true } } },
  ExecuteSemanticQueryInput: { fields: { queryMode: "AUTO 自然语言查询；FIXED_SHAPE 明确查询结构；ANALYSIS 仅返回分析任务与上下文。", namespace: "本体命名空间。", ontologyVersion: "发布版本号或 latest；有 sessionId 时须与会话版本一致。", question: "AUTO 与 ANALYSIS 使用的自然语言问题。", queryShape: "FIXED_SHAPE 必填。包含 rootObjectId、measureIds、dimensionPropertyIds、filters、sort 等；具体 ID 从本体或语义上下文取得。", parameters: "查询结构中参数占位符的名称与取值。", sessionId: "语义上下文返回的会话 ID，用于固定版本和解析短引用。", pagination: "pageSize 每页 1–10000 行；下一页使用 completeness.nextCursor，保持查询和参数一致。", options: "可开启 includeResolution、includeOntologyContext、includeAxioms、includeInferenceEvidence、includeQueryIr、includeSqlPreview。" }, example: { namespace: "retail", ontologyVersion: "latest", queryMode: "ANALYSIS", question: "按店铺查看销售额", options: { includeAxioms: true, includeInferenceEvidence: true } } },
